...
import { addPost, cancelEditPost, updateFinishPost } from 'pages/blog/blog.slice'
...
export default function CreateBlog() {
  ...
  //Lấy dữ liệu đã đưuọc edit ra
  const editPost = useSelector((state: RootState) => state.blog.editingPost)
  //Sau khi ta đã dispatch edit từ postList và đã trả về dữ liệu. Ta sẽ lấy nó ra và hiển thị lên màn hình
  //Nếu có editPost thì set vào setFormData
  React.useEffect(() => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editPost) {
      //Nếu đang là edit thì xử lý thằng này để cập nhật giá trị sau khi thay đổi
      dispatch(updateFinishPost(formData))
    } else {
      //Ban đầu khi post sẽ koh có id nên h ta sẽ cập nhật một id ngẫu nhiên
      const formDataWidthId = { ...formData }
      //Vì ta dùng nanoid nên ta đã bỏ id vừa tạo
      dispatch(addPost(formDataWidthId))
      //1. dispatch một actions có tên là addPost. Sau đó vào reducer
    }
  ...
  // const handleFinishUpdatePost = (post: Post) => {
  //   dispatch(updateFinishPost(post))
  // }
  return (
    <>
      <form onSubmit={handleSubmit}>
        <div className='mb-6'>
          ...
          <input
            ...
            //giữ lại giá trị cũ trước khi update giá trị mới
            required
          />
        </div>
        <div className='mb-6'>
        ...
        <div>
          {/* Nếu có editPost thì hiển thị hai nút update và cancel */}
          {editPost && (
            <Fragment>
              <button
          ...
          {/* Nếu không có editPost thì hiển thị nut post */}
          {!editPost && (
            <button
              type='submit'
